...
typedef enum
{
	PGSP_ADVICE_OFF,			/* never collect pg_plan_advice strings */
	PGSP_ADVICE_ALWAYS,			/* collect advice for every tracked plan */
	PGSP_ADVICE_DELAYED,		/* collect advice only on the second occurrence
								 * of a query id, using cross-connection
								 * tracking (Postgres 19+ only) */
}			PGSPAdviceMode;

static const struct config_enum_entry advice_options[] =
{
	{"off", PGSP_ADVICE_OFF, false},
	{"always", PGSP_ADVICE_ALWAYS, false},
	{"delayed", PGSP_ADVICE_DELAYED, false},
	{NULL, 0, false}
};
...
static int	pgsp_plan_advice = PGSP_ADVICE_OFF; /* pg_plan_advice collection */
...
typedef struct PgStatShared_PlanInfo
{
	...
	dsa_pointer plan_advice;	/* DSA memory containing the pg_plan_advice
								 * string (Postgres 19+), or InvalidDsaPointer */
...
pgsp_dealloc_entries(void)
{
	...
	for_each_from(lc, entries, list_length(entries) - nvictims)
	{
		...
		if (p != NULL)
		{
			...
			{
				dsa_free(pgStatCustomLocal.dsa, statent->info.plan_advice);
...
/*
 * Shared set of query ids that are still missing a plan advice string, used by
 * the "delayed" advice mode. Membership is the flag: an entry exists if and
 * only if we want pg_plan_advice to generate advice the next time the query is
 * planned (on any connection). Backed by a named dshash in the DSM registry,
 * so no extra shared memory plumbing is required.
 */
typedef struct PgspAdviceWantedKey
{
	Oid			dboid;
	uint64		queryid;
}			PgspAdviceWantedKey;

static const dshash_parameters advice_wanted_params = {
	sizeof(PgspAdviceWantedKey),
	sizeof(PgspAdviceWantedKey),
	dshash_memcmp,
	dshash_memhash,
	dshash_memcpy,
	0							/* tranche_id is ignored by GetNamedDSHash() */
};

static dshash_table *advice_wanted_hash = NULL;

static dshash_table *
pgsp_advice_wanted_attach(void)
{
	if (advice_wanted_hash == NULL)
	{
		bool		found;

		advice_wanted_hash = GetNamedDSHash("pg_stat_plans_advice_wanted",
											&advice_wanted_params, &found);
	}

	return advice_wanted_hash;
}

static void
pgsp_advice_wanted_init_key(PgspAdviceWantedKey *key, uint64 queryId)
{
	/* zero out padding so memcmp/memhash are deterministic */
...
static bool
pgsp_advice_wanted(uint64 queryId)
{
	dshash_table *h = pgsp_advice_wanted_attach();
	PgspAdviceWantedKey key;
	void	   *entry;

	pgsp_advice_wanted_init_key(&key, queryId);
	entry = dshash_find(h, &key, false);
	if (entry != NULL)
	{
		dshash_release_lock(h, entry);
		return true;
	}
...
static void
pgsp_advice_wanted_add(uint64 queryId)
{
	dshash_table *h = pgsp_advice_wanted_attach();
	PgspAdviceWantedKey key;
	bool		found;
	...
	pgsp_advice_wanted_init_key(&key, queryId);
	entry = dshash_find_or_insert(h, &key, &found);
	dshash_release_lock(h, entry);
}

static void
pgsp_advice_wanted_remove(uint64 queryId)
{
	dshash_table *h = pgsp_advice_wanted_attach();
	PgspAdviceWantedKey key;

	pgsp_advice_wanted_init_key(&key, queryId);
	(void) dshash_delete_key(h, &key);
}
						 ...
						 double exec_time)
{
	 ...
	 *
	 * In "delayed" mode the first occurrence of a query id is not planned with
	 * advice generation enabled, so no advice is available yet; we flag the
	 * query id as wanting advice so the next planning (on this or any other
	 * connection) generates it, and we record it then.
	 */
	if (pgsp_plan_advice != PGSP_ADVICE_OFF &&
		!DsaPointerIsValid(shstatent->stats.info.plan_advice))
	{
		...
		if (advice != NULL && advice[0] != '\0')
		{
			...
			{
				dsa_pointer advice_dp = dsa_allocate(pgStatCustomLocal.dsa, advice_size);

				memcpy(dsa_get_address(pgStatCustomLocal.dsa, advice_dp), advice, advice_size);
			...
			/* We have advice for this query id now, stop requesting it */
			if (pgsp_plan_advice == PGSP_ADVICE_DELAYED)
				pgsp_advice_wanted_remove(queryId);
		}
		else if (pgsp_plan_advice == PGSP_ADVICE_DELAYED)
		{
			/* No advice yet; request generation on the next planning */
			pgsp_advice_wanted_add(queryId);
		}
	}
#endif
...
#endif
{
	 ...
	 * for this query. We only do this for queries we would actually track, and
	 * in "delayed" mode only for query ids that are still missing advice (as
	 * recorded in the shared advice-wanted set, so a second execution on any
	 * connection can capture it). The advice ends up in
	 * PlannedStmt.extension_state and is recorded in pgstat_report_plan_stats.
	 *
		...
		pgsp_enabled(nesting_level))
	{
		...
		if (advice_fn != NULL)
			advise = (pgsp_plan_advice == PGSP_ADVICE_ALWAYS) ||
				pgsp_advice_wanted(parse->queryId);
	}
...
_PG_init(void)
{
	...
	DefineCustomEnumVariable("pg_stat_plans.plan_advice",
							 "Controls collection of pg_plan_advice strings for tracked plans.",
							 "Only effective on Postgres 19 or newer with pg_plan_advice loaded. "
							 "\"delayed\" records advice on the second execution of a query id.",
							 &pgsp_plan_advice,
							 PGSP_ADVICE_OFF,
							 advice_options,
							 PGC_SUSET,
...
#if PG_VERSION_NUM >= 190000
	/* Also clear the shared set of query ids still missing plan advice */
	if (pgsp_plan_advice == PGSP_ADVICE_DELAYED || advice_wanted_hash != NULL)
	{
		dshash_table *h = pgsp_advice_wanted_attach();
		dshash_seq_status s;
		void	   *entry;

		dshash_seq_init(&s, h, true);
		while ((entry = dshash_seq_next(&s)) != NULL)
			dshash_delete_current(&s);
		dshash_seq_term(&s);
	}
#endif
...
#define PG_STAT_PLANS_COLS 9
	...
	while ((p = dshash_seq_next(&hstat)) != NULL)
	{
		...
		{
			char	   *astr = dsa_get_address(pgStatCustomLocal.dsa, statent->info.plan_advice);
